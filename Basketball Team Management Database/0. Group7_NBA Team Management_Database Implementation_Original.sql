...
CREATE TABLE dbo.Team
	...
	--delete in erd
	--RankInRegion INT NOT NULL,
	--delete in erd
	--RankInNBA INT NOT NULL,
...
AS
	SELECT RANK() OVER (ORDER BY AVG(gt.TeamScore) DESC) AS [Rank],
		TeamName, Region,AVG(gt.TeamScore) AS [TeamAvgScore] 
...
AS
	SELECT RANK() OVER (ORDER BY AVG(gp.PlayerScore) DESC) AS [Rank], 
		PlayerFirstName, PlayerLastName, AVG(gp.PlayerScore) AS [PlayerAvgScore]
...
AS
	SELECT TeamName, 
		   COUNT(CASE WHEN gt.WinOrLose = 'W' THEN 1 END) AS GameWon,
		   RANK() OVER (ORDER BY 
				  COUNT(CASE WHEN gt.WinOrLose = 'W' THEN 1 END) DESC) AS [RankInNBA]
	FROM dbo.GameTeamPlayed gt
	JOIN dbo.Team t
	ON gt.TeamID = t.TeamID
	GROUP BY t.TeamName;
...
AS
	SELECT	TeamName,
			...
			RANK() OVER (PARTITION BY Region ORDER BY
				   COUNT(CASE WHEN gt.WinOrLose = 'W' THEN 1 END) DESC) 
			AS [RankInRegion]
	FROM dbo.GameTeamPlayed gt
	JOIN dbo.Team t
	ON gt.TeamID = t.TeamID
...
ALTER TABLE dbo.SponsorToPlayer 
	ADD CONSTRAINT BanMoreSponsorToPlayer
	CHECK (dbo.CheckSponsorToPlayer(PlayerID) <= 1);
...
-- Housekeeping
ALTER TABLE dbo.SponsorToPlayer 
	DROP CONSTRAINT BanMoreSponsorToPlayer;
...
AS
	BEGIN
	  ...
	  SELECT @WinOrLose = 
		  CASE WHEN 
			   @teamID = (SELECT Temp.TeamID 
						  FROM (SELECT TOP 1 TeamID, TeamScore
...
SELECT GameID, TeamID, TeamScore, WinOrLose
From GameTeamPlayed;
